...
const AdminDoctors = () => {
  // allDoctors holds the full unfiltered list; filtered view is derived from it
  ...
  // Counts computed from the single source of truth
  const counts = {
  ...
  // Displayed list is client-side filtered
  ...
  const doAction = async (actionFn, id) => {
    ...
    try {
      await actionFn(id);
      // Re-fetch the full list so counts and filter both stay accurate
      await fetchAll();
    } catch {
      // ignore
    } finally {
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        ...
        {loading ? (
          ...
          <ul className="divide-y divide-gray-50">
            {doctors.map((d) => (
              <li key={d._id} className="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="font-semibold text-gray-800">Dr. {d.name}</p>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_BADGE[d.accountStatus]}`}>
                ...
                <div className="flex gap-2 flex-wrap">
                  {d.accountStatus === 'pending_verification' && (
                    <ActionBtn color="green" loading={loadingId === d._id} onClick={() => doAction(adminService.verifyDoctor, d._id)}>
                      Verify
                    </ActionBtn>
                  )}
                  {d.accountStatus === 'verified' && (
                    <ActionBtn color="orange" loading={loadingId === d._id} onClick={() => doAction(adminService.suspendDoctor, d._id)}>
                      Suspend
                    </ActionBtn>
                  )}
                  {d.accountStatus === 'suspended' && (
                    <ActionBtn color="green" loading={loadingId === d._id} onClick={() => doAction(adminService.reinstateDoctor, d._id)}>
                      Reinstate
                    </ActionBtn>
                  )}
                </div>
