      ...
      <div className="space-y-6 pb-12">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24">
            ...
            <p className="text-sm text-slate-400">{t({ en: 'Loading logs...', hi: 'लॉग लोड हो रहे हैं...' })}</p>
          </div>
        ) : logs.length === 0 ? (
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800 py-20 text-center">
            ...
            <p className="text-slate-500 dark:text-slate-400 font-semibold">{t({ en: 'No audit logs found', hi: 'कोई ऑडिट लॉग नहीं मिले' })}</p>
          </div>
        ) : (
          <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200/60 dark:border-slate-800 overflow-hidden shadow-sm">
            <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800">
              <h2 className="text-sm font-bold text-slate-900 dark:text-white">{t({ en: 'Activity Trail', hi: 'गतिविधि ट्रेल' })}</h2>
            </div>
                    ...
                    <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                      ...
                      {log.patient?.name && (
                        <span className="text-[11px] text-slate-400 dark:text-slate-500">
                          {t({ en: 'Patient', hi: 'मरीज' })}: <span className="font-medium text-slate-600 dark:text-slate-300">{log.patient.name}</span>
                        </span>
                      )}
                    </div>
                    <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed">{log.details || t({ en: 'No details', hi: 'कोई विवरण नहीं' })}</p>
                    <p className="text-[11px] text-slate-400 dark:text-slate-500 mt-1.5">
                      {t({ en: 'By', hi: 'द्वारा' })} <span className="font-medium">{log.performedBy?.name || t({ en: 'System', hi: 'सिस्टम' })}</span>
                      <span className="opacity-60"> ({log.performedBy?.role || t({ en: 'N/A', hi: 'उपलब्ध नहीं' })})</span>
        ...
        {totalPages > 1 && (
          <div className="flex justify-center items-center gap-3">
            ...
            <span className="text-xs font-medium text-slate-500 dark:text-slate-400">
              {t({ en: 'Page', hi: 'पेज' })} {page} {t({ en: 'of', hi: 'का' })} {totalPages}
            </span>
            <button
