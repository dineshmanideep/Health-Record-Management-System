        ...
        <div className="bg-white dark:bg-slate-900 p-10 rounded-[3rem] shadow-xl border-2 border-indigo-500/20 mb-8 animate-in zoom-in-95 duration-300">
          ...
          {affiliateMsg.text && (
            <div className={`mt-8 p-4 rounded-xl text-[10px] font-black uppercase tracking-widest text-center animate-in slide-in-from-top-2 duration-300 ${
              affiliateMsg.type === 'success' ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400' : 'bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400'
            }`}>
              {affiliateMsg.type === 'success' ? '✅' : '⚠️'} {affiliateMsg.text}
            </div>
          )}
        </div>
      )}
